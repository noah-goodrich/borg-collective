...
- [ ] D1 — All 62 silently-skipped assertions are enforced on macOS. Convert non-final `[[ ]]` to POSIX `[ ]`
...
- [ ] D2 — shellcheck covers every `.sh` file, not just `hooks/`. `lib/*.sh` and `bin/*` (those with a sh/bash
      shebang) are linted.
  - Verify: `.github/workflows/test.yml` lint job includes `lib/*.sh`; CI lint job passes.
- [ ] D3 — The bats harness is pinned. No `clone … master`.
  - Verify: `grep -n 'bats' .github/workflows/test.yml` shows a pinned tag/SHA or
    `bats-core/bats-action@<version>`; CI test job passes.
- [ ] D4 — A scheduled workflow re-runs lint+test against `main` independently of PR activity, so a red main
      surfaces without anyone looking.
  - Verify: a workflow with `on: schedule` exists and its first run appears in the Actions tab.
- [ ] D5 — Branch protection on `main`: required status checks (`lint`, `test`), require branches up to date
...
- [ ] B1 — A `tests/cli_contract.bats` suite invokes the real `borg.zsh` as a subprocess under **zsh** and
...
- [ ] B2 — At least one contract test would have caught each of this session's three bug classes. Documented
...
- [ ] B3 — The contract suite runs on a **`macos-latest`** CI leg. Only the contract suite runs there, not the
...
- [ ] B4 — Regression: full suite still green in CI, and the macOS leg green.
  - Verify: `gh pr checks` shows all jobs pass.

### Honesty
- [ ] H1 — `tests/README.md` (new) states plainly what each lane proves and does not prove: the bash lane
