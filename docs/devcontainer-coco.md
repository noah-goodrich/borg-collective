...
### Devcontainer Lifecycle: postCreateCommand vs postStartCommand

`drone` runs **both** lifecycle hooks, not just one — each in a different role:

- **`postCreateCommand`** runs **once**, at container creation. It handles one-time dependency
  install (e.g. `pip install -e '.[dev]'`, `npm install`) plus the initial dotfile symlinks.
- **`postStartCommand`** runs on **every start** (`drone up`, restarts, rebuilds). It re-applies
  per-start setup that must survive container restarts: dotfile symlinks, `mkdir`/`chmod` for the
  SSH agent socket, and the `/workspace` symlink recreation.

Both are generated by `drone.zsh` when scaffolding a project's `devcontainer.json`; neither is
optional or unused.
