...
is the record the branch used to carry as its own `PROJECT_PLAN.md`; it is a directive now because the
repository has exactly one plan slot and One Front Door holds it. Ship it through `/borg-assimilate` as a
child of that plan, not as a second plan.
...
the One Front Door plan; that was in tension with replacing `PROJECT_PLAN.md`, because the repository has
exactly ONE plan slot at root and `manifest.cli resolve` rule 3 reads exactly one `- Plan-slug:` from it.
Two independent plans could not both be that file, and three live readers depend on the current one —
`resolve` rule 3, the `planstate` reconciliation in `/borg-link-up`, and the directives parented to its
slug. Of the three ways out recorded here on 2026-09-17 (merge after One Front Door ships; demote to a
directive; a second plan slot), the second is the repository's actual convention for side work and is
the one taken. Consequences, all applied on the branch the same day: `PROJECT_PLAN.md` is byte-identical
to main's again; this file carries the criteria, evidence and ticks; the `viz-program` manifest row for
#220 says it files a directive; and #220 merges as a directive filing rather than a plan replacement, so
the "does not merge while One Front Door's AC5 is in flight" condition no longer applies for the
plan-slot reason. Its other conditions — CI on five lanes, both machines' stamps — still do.
...
- ~~**The nine orphans.**~~ **Ruled out of scope by Noah, 2026-09-17; moot since 2026-09-19**, when the
  plan file went back to main's. Replacing the plan file
...
- **AC4 requires work in a private repo from a public one.** The plan describes it; the diff cannot
  live here. (Mislabelled AC5 until 2026-09-18; this directive's AC5 is nothing-breaks.) `borg-collective`
  was history-scrubbed once already for employer references.
