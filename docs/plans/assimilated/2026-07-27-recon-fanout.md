...
The engine must be **source-agnostic**. Ontra-specific sources (Slack/Jira/Notion/Ontra) are NOT
built here — they are injected on a work machine via a pluggable adapter interface. The primitive
works for any project on any machine, Ontra or not. This repo ships exactly one reference adapter
...
This is how an Ontra layer gets injected on the work machine without touching this repo.
...
- [ ] Ontra source adapters (Slack/Jira/Notion) — explicitly OUT of scope here; a separate layer.

## Key design decisions / trade-offs

1. **Adapters = executables on a search path, not a plugin registry.** Zero coupling: a machine adds
   a source by dropping a file; the repo never learns about Ontra sources. Trade-off: the contract is
