...
*Shipped: 2026-04-14 — direct push to main, tagged v0.7.0*
...
- [x] **Project session colors** — registry gains `color` field; `borg color <project> <name>`
...
- [x] **Cortex Code parity in drone** — `drone cortex [project]` launches Cortex Code mirroring
...
- [x] **Verify Cortex reads Claude skills natively** — confirm or deny whether Cortex CLI reads
...
Beyond the stated criteria, the following was delivered in this release:

- **`/simplify` enforcement in borg-assimilate** — Step 0 added to the assimilate skill requiring
  simplify before any shipping evaluation.
- **Backward-compatible debrief fallback** — `borg-start.sh` checks project-local `.borg/debriefs/`
  first, falls back to global `~/.config/borg/debriefs/PROJECT.md` for pre-migration projects.
- **Cairn unavailability surfacing** — session start warns explicitly when cairn is not in PATH.
- **`ANTHROPIC_SDK_KEY` alias** — stop hook forwards `ANTHROPIC_SDK_KEY` to `ANTHROPIC_API_KEY`
  so debrief generation works regardless of which env var is set.
- **`.borg-project` marker resolution** — hooks resolve project name from marker file walking up
  ancestor directories, enabling correct project identification inside drone containers.
