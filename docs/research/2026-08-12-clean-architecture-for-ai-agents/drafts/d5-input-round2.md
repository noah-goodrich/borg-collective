# D5 Round 2 blind-review input — problem statement, revised option set, chosen option only

This is a SECOND, independent blind review. A first review (Round 1) already found real problems
with an earlier version of this option set and forced a revision — the option set below already
reflects that correction. Review it cold, as if for the first time; do not assume the correction is
sufficient just because a round already happened.
...
### Option B (revised, Round 2): Vertical-slice per command + import-linter Independence contract
...
- **Pros:** Matches the one piece of directly-verified real-world precedent found (Anthropic's own
  ...
  both tools are existing, already-installed, already-inspected PyPI packages — no new bespoke
  tooling, just correct composition of two tools instead of assuming one does both jobs.
- **Cons:** Still a two-file minimum per command, even for a trivially small one; whole-module
  ...
  timestamp-generating shell call; now requires configuring and maintaining TWO linter configs
  instead of one, and the `ResourceChecker`'s hardcoded internal-module allowlist (`domain`, `dto`,
  `use_cases`, `protocols`, `models`, `telemetry` — none of which match `core`/`shell`) means a
  `borg_core.*` prefix will likely need adding to `allowed_prefixes` or intra-package imports inside
  a single command's own `core.py`/`shell.py` pair will themselves be flagged as forbidden.
- **Key tradeoffs:** Forces every command, however small, to pay a 2-file tax, now paired with a
  2-tool config tax. Correct, but no longer "single knob, zero new config."
- **Feasibility:** High but more involved than Round 1 claimed — both mechanisms are proven to exist
  and do what's needed, but the composition (two tools, one allowlist fix) is unverified in
  combination until actually configured against a real two-command scratch repo.
- **Estimate:** Matches Part 3's existing C1-C7 estimate for the `recon` migration, plus ~0.5 session
  to configure and smoke-test the `import-linter` Independence contract and the `allowed_prefixes`
  fix (not previously budgeted — a direct cost of the Round 2 correction).
...
### Option G (added Round 2, per D5 Ideator): Dynamic/runtime I/O interception
- **What it is:** A single-file, fully colocated command (no forced `core.py`/`shell.py` split at
  all). A pytest fixture or decorator marks specific functions as `@core`; at TEST-RUN TIME, that
  ...
  executing the code, not by resolving a static layer label.
...
## Chosen option (Round 2)

The draft recommendation selects **Option B (revised)** — vertical-slice per command, colocated
core.py/shell.py pair, module_map-tagged for the raw-I/O check — PLUS an import-linter Independence
contract for cross-command isolation (deferred until a second command exists to isolate recon from),
PLUS a verified allowed_prefixes fix for intra-package imports. Option F complexity-gate PRINCIPLE is
stated in the pattern doc from day one (the CI automation for the gate itself is deferred). Option G
(dynamic/runtime I/O interception) is recorded as a deferred complementary layer, not the primary
mechanism.
