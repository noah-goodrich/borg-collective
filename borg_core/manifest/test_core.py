...
@pytest.mark.parametrize(
    "ref",
    ["ingle#12", "DE-123", "o/r", "o/r#", "o/r#abc", "a/b/c#1", "/r#1", "o/#1", "o r/x#1"],
    ids=[
        "repo-shorthand",
        "jira-key",
        "no-number",
...
def test_declared_refs_keeps_refs_that_are_not_full_refs():
    ...
    assert core.declared_refs(_manifest([_row("1", "DE-123")])) == ["DE-123"]
...
def test_select_for_repository_returns_nothing_for_an_empty_slug():
    ...
    # with the guard deleted. `DE-123` makes ref_slug("") == "" == slug, so only the guard stops it.
    m = _manifest([_row("1", "DE-123")])
    assert core.ref_slug("DE-123") == "", "the precondition that makes this test discriminate"
...
def test_select_for_repository_ignores_refs_that_do_not_parse():
    m = _manifest([_row("1", "DE-123")])
    assert core.select_for_repository([m], "DE-123") == []
