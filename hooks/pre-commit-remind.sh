#!/usr/bin/env bash
# pre-commit-remind.sh — PreToolUse hook: nudge to run /simplify and /borg-ship before commit
...
case "$COMMAND" in
    *"git commit"*)
        ...
        if [[ -n "$CWD" && -f "$CWD/PROJECT_PLAN.md" ]]; then
            MSG="$MSG Also: have you run /borg-ship to verify acceptance criteria are met?"
        fi
        jq -n --arg msg "$MSG" '{
            "hookSpecificOutput": {
